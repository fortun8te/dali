...
device_streaming_cb(struct output_device *device, enum output_device_state status)
{
  if (!device)
	...
	goto out;
      ...
      // dead while the healthy speaker keeps streaming (MICHAEL D pattern in
